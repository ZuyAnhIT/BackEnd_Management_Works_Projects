...
import com.quanlyduan.project_manager_api.model.AuthToken; // Entity AuthToken
...
@Repository
/**
 * Repository cho Entity AuthToken (Quản lý Refresh Token, OTP, Reset Token).
 */
public interface AuthTokenRepository extends JpaRepository<AuthToken, Integer> {

    /**
     * Tìm kiếm Token theo giá trị chuỗi và loại Token (ví dụ: tìm Refresh Token hoặc OTP).
     */
    Optional<AuthToken> findByTokenAndTokenType(String token, TokenType tokenType);

    /**
     * Thu hồi (vô hiệu hóa) tất cả REFRESH token đang ACTIVE của một người dùng.
